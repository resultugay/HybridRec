...
data = pd.DataFrame({'user_id': pd.Series(aday_id),
                     'item_id': pd.Series(ilan_id),
                     'ratings': pd.Series(ratings),
                     'ts': pd.Series(ts)})[['user_id', 'item_id', 'ratings', 'ts']]
...
# Let
numeric = ['user_id', 'item_id', 'ratings', 'ts']
data[numeric] = data[numeric].apply(pd.to_numeric, errors='coerce')

sortedbyid = data.sort_values(['user_id', 'ts'], ascending=[True, False])

sortedbyid.to_csv(os.path.join(TPS_DIR, 'user_item.csv'), index=False,header=None)
